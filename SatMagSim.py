...
class Constants:
    ...
    SEPERATION_TIME = 10 #SEC
    DEPLOYMENT_TIMER = 20 #MIN
...
class SpacecraftGUI(CTk):
    ...
    def run_simulation(self):
        ...
        for x in range(int(num_steps / step)):
            ...
            simulator = SatelliteSimulator(
                ...
                N=Constants.DISTURBANCE_TORQUES,
    ...
    def create_deployment_properties_frame(self, parent):
        ...
        labels = ["Seperation (sec)", "Deployment Timer (min)"]
...
# Create satellites with different true anomalies
ta = Constants.TRUE_ANOMALIES
satellite = create_satellite(name="Taurus", ta=ta)
...
class SatelliteSimulator:
    ...
    def w_and_q(self, t, y, Btot_body, x):
        ...
        if x <= int(Constants.DEPLOYMENT_TIMER * 60 / Constants.STEP):
            ...
            torque_RG[0] = 0  # Set torque on X axis to zero
            torque_RG[1] = 0  # Set torque on Y axis to zero
    ...
    def integrate_w_and_q(self, x, w, q, Btot_body, step):
        ...
        sol = solve_ivp(self.w_and_q, [0, step], y0, args=(Btot_body, x), method='DOP853', rtol=1e-9, atol=1e-9, t_eval=[step])
    ...
    def calculate_magnetic_fields(self, gator, satellites, initial_ut, current_time, kp_index, x, num_steps, step, data_dyn_kin, data_PV, data_geodetic, data_magnetic):
        start_time = time.time()  # Start timing
        ...
        if x >= int(Constants.SEPERATION_TIME / Constants.STEP):
            ...
            # İlk 10 adımda w ve q sabit kalıyor
            w_sol, q_sol = w, q
